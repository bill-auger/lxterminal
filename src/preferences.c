...
void terminal_preferences_dialog(GtkAction * action, LXTerminal * terminal)
{
    ...
    w = GTK_WIDGET(gtk_builder_get_object(builder, "hide_tabs"));
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(w), setting->hide_tabs);
    g_signal_connect(G_OBJECT(w), "toggled",
        G_CALLBACK(preferences_dialog_generic_toggled_event), &setting->hide_tabs);
